...
from lasagne.objectives import categorical_accuracy, multiclass_hinge_loss
...
from margin import margin_sensitivity
...
flags.DEFINE_float("lmbd", 0.1, "regularization coefficient")
flags.DEFINE_integer("margin_order", 2, "order of margin norm maximization")
...
def main():
    ...
    # losses
    train_ma = multiclass_hinge_loss(train_outputs['logits'], train_labels).mean()
    if FLAGS.margin_order == 2:
        train_ma_sens = margin_sensitivity(train_images, train_outputs['logits'],
                                           train_labels, num_classes).mean()
    else:
        train_ma_sens = margin_sensitivity(train_images, train_outputs['logits'],
                                           train_labels, num_classes, ord=np.inf).mean()
    ...
    train_metrics = OrderedDict([('loss', train_loss),
                                 ...
                                 ('err', train_err)])
    val_metrics = OrderedDict([('margin', val_ma),
                               ('err', val_err)])

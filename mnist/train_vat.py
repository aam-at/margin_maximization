...
from utils import build_result_str, save_network, setup_train_experiment
...
flags.DEFINE_float("epsilon", 2.0, "epsilon for generative vat perturbation")
flags.DEFINE_integer("num_iterations", 1, "number of iterations for vat")
...
def main():
    ...
    train_vat = virtual_adversarial_training(
        lambda x: model(x)['logits'], train_images, train_outputs['logits'],
        FLAGS.epsilon, num_iterations=FLAGS.num_iterations).mean()
    train_loss = train_ce + FLAGS.lmbd * train_vat
    ...
    try:
        ...
        save_image(
            torch.from_numpy(summary_images), save_path,
            int(np.sqrt(summary_images.shape[0])))
        ...
        while epoch < FLAGS.num_epochs:
            ...
            for batch_index, (images, labels) in enumerate(train_iterator, 1):
            ...
            if epoch % FLAGS.summary_frequency == 0:
                ...
                save_image(
                    torch.from_numpy(summary[-1]), save_path,
